...
//            parameters.setPictureSize(parameters.getPreviewSize().width,parameters.getPictureSize().height);
     ...
     * 拍照结果回调
     */
    public void doTakePic(TakePicListener listener) {
        if (isPreview && mCamera != null) {
            mCamera.takePicture(new ShutCallBackImpl(), null, new PicCallBacKImpl(listener));
        }
    }
    ...
    private class PicCallBacKImpl implements Camera.PictureCallback {
        ...
        private TakePicListener listener;
        private Handler handler;
        public PicCallBacKImpl(TakePicListener listener) {
...
//            isPreview = true;
    ...
    public void startPreview() {
        if (mCamera!=null&&isPreview==false){
            mCamera.startPreview();//重新开启预览 ，不然不能继续拍照
            isPreview = true;
        }
    }
